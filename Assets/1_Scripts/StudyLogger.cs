...
public class StudyLogger : MonoBehaviour
{
    ...
    public void StartLogging(string participantID, string taskID, string conditionID, string blockID)
    {
        if (isLogging) return;
        ...
        sessionLog = new FileWriter(sessionFolderName, $"{baseFileName}_SessionLog.csv", "TotalTimeSeconds");
        quizLog = new FileWriter(sessionFolderName, $"{baseFileName}_QuizLog.csv", "Timestamp, QuestionIndex,SelectedAnswer,IsCorrect");
        interactionLog = new FileWriter(sessionFolderName, $"{baseFileName}_InteractionLog.csv", "Timestamp,InteractionType,TargetObject,Details");
        movementLog = new FileWriter(sessionFolderName, $"{baseFileName}_MovementLog.csv", "Timestamp,HeadPosX,HeadPosY,HeadPosZ,HeadRotX,HeadRotY,HeadRotZ,HeadRotW");
